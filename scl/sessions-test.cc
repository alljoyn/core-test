...
/* constants */
static const char* TEST_SERVICE_INTERFACE_NAME1 = "org.alljoyn.int1";
static const char* TEST_SERVICE_INTERFACE_NAME2 = "org.alljoyn.int2";
static const char* TEST_SERVICE_OBJECT_PATH1 = "/sessionsbo1";
static const char* TEST_SERVICE_OBJECT_PATH2 = "/sessionsbo2";
...
class SessionTestObject : public BusObject {
  public:
    ...
    /** Send a Chat signal */
    void SendChatSignal11(SessionId id, const char* chat, uint8_t flags)
    {
        MsgArg chatArg("s", chat);
        Message msg(*bus);
        QStatus status = Signal(NULL, id, *chatSignalMember11, &chatArg, 1, ttl, flags, &msg);
        if (status == ER_OK) {
            printf("Sent chat signal with serial = %d\n", msg->GetCallSerial());
        } else {
            printf("Failed to send chat signal (%s)\n", QCC_StatusText(status));
    ...
    /** Send a Chat signal */
    void SendChatSignal12(SessionId id, const char* chat, uint8_t flags)
    {
        MsgArg chatArg("s", chat);
        Message msg(*bus);
        QStatus status = Signal(NULL, id, *chatSignalMember12, &chatArg, 1, ttl, flags, &msg);
        if (status == ER_OK) {
            printf("Sent chat signal with serial = %d\n", msg->GetCallSerial());
        } else {
            printf("Failed to send chat signal (%s)\n", QCC_StatusText(status));
    ...
    /** Send a Chat signal */
    void SendChatSignal21(SessionId id, const char* chat, uint8_t flags)
    {
        MsgArg chatArg("s", chat);
        Message msg(*bus);
        QStatus status = Signal(NULL, id, *chatSignalMember21, &chatArg, 1, ttl, flags, &msg);
        if (status == ER_OK) {
            printf("Sent chat signal with serial = %d\n", msg->GetCallSerial());
        } else {
            printf("Failed to send chat signal (%s)\n", QCC_StatusText(status));
    ...
    /** Send a Chat signal */
    void SendChatSignal22(SessionId id, const char* chat, uint8_t flags)
    {
        MsgArg chatArg("s", chat);
        Message msg(*bus);
        QStatus status = Signal(NULL, id, *chatSignalMember22, &chatArg, 1, ttl, flags, &msg);
        if (status == ER_OK) {
            printf("Sent chat signal with serial = %d\n", msg->GetCallSerial());
        } else {
            printf("Failed to send chat signal (%s)\n", QCC_StatusText(status));
  ...
  private:
    ...
    const InterfaceDescription::Member* chatSignalMember11;
    const InterfaceDescription::Member* chatSignalMember12;
    const InterfaceDescription::Member* chatSignalMember21;
    const InterfaceDescription::Member* chatSignalMember22;
    uint32_t ttl;
};
...
int TestAppMain(int argc, char** argv)
{
    ...
    /* Create org.alljoyn.bus.test.sessions interface 1 */
    InterfaceDescription* testIntf1 = NULL;
    status = bus->CreateInterface(TEST_SERVICE_INTERFACE_NAME1, testIntf1);
    if (ER_OK == status) {
        testIntf1->AddSignal("Chat", "s",  "str", 0);
        testIntf1->AddSignal("Chat11", "s",  "str", 0);
        testIntf1->AddSignal("Chat12", "s",  "str", 0);
        testIntf1->Activate();
    } else {
        printf("Failed to create interface \"%s\" (%s)\n", TEST_SERVICE_INTERFACE_NAME1, QCC_StatusText(status));
    }

    /* Create org.alljoyn.bus.test.sessions interface 2 */
    InterfaceDescription* testIntf2 = NULL;
    status = bus->CreateInterface(TEST_SERVICE_INTERFACE_NAME2, testIntf2);
    if (ER_OK == status) {
        testIntf2->AddSignal("Chat21", "s",  "str", 0);
        testIntf2->AddSignal("Chat22", "s",  "str", 0);
        testIntf2->Activate();
    } else {
        printf("Failed to create interface \"%s\" (%s)\n", TEST_SERVICE_INTERFACE_NAME2, QCC_StatusText(status));
    }

    /* Create and register the bus object 1 that will be used to send and receive signals */
    SessionTestObject sessionTestObj1(*bus, TEST_SERVICE_OBJECT_PATH1);
    bus->RegisterBusObject(sessionTestObj1);

    /* Create and register the bus object 2 that will be used to send and receive signals */
    SessionTestObject sessionTestObj2(*bus, TEST_SERVICE_OBJECT_PATH2);
    bus->RegisterBusObject(sessionTestObj2);
    ...
    status =  s_bus->RegisterSignalHandler(&signalReceiver,
                                           static_cast<MessageReceiver::SignalHandler>(&SignalReceiver::ChatSignalHandler),
                                           chatSignalMember,
                                           NULL);
    /* Register signal handler */
    status =  s_bus->RegisterSignalHandler(&signalReceiver,
    ...
    while ((ER_OK == status) && (get_line(buf, bufSize, stdin))) {
        ...
        if (cmd == "debug") {
            ...
            AutoChatThread::Launch(sessionTestObj1, id, count, delay, minSize, maxSize);
        ...
        } else if (cmd == "schat1.1.1") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat1.1.1 <msg>\n");
                continue;
            }
            sessionTestObj1.SendChatSignal11(0, chatMsg.c_str(), flags);
        } else if (cmd == "schat1.1.2") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat1.1.2 <msg>\n");
                continue;
            }
            sessionTestObj1.SendChatSignal12(0, chatMsg.c_str(), flags);
        } else if (cmd == "schat1.2.1") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat1.2.1 <msg>\n");
                continue;
            }
            sessionTestObj1.SendChatSignal21(0, chatMsg.c_str(), flags);
        } else if (cmd == "schat1.2.2") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat1.2.2 <msg>\n");
                continue;
            }
            sessionTestObj1.SendChatSignal22(0, chatMsg.c_str(), flags);
        } else if (cmd == "schat2.1.1") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat2.1.1 <msg>\n");
                continue;
            }
            sessionTestObj2.SendChatSignal11(0, chatMsg.c_str(), flags);
        } else if (cmd == "schat2.1.2") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat2.1.2 <msg>\n");
                continue;
            }
            sessionTestObj2.SendChatSignal12(0, chatMsg.c_str(), flags);
        } else if (cmd == "schat2.2.1") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat2.2.1 <msg>\n");
                continue;
            }
            sessionTestObj2.SendChatSignal21(0, chatMsg.c_str(), flags);
        } else if (cmd == "schat2.2.2") {
            uint8_t flags = ALLJOYN_FLAG_SESSIONLESS;
            String chatMsg = Trim(line);
            if (chatMsg.empty()) {
                printf("Usage: schat2.2.2 <msg>\n");
                continue;
            }
            sessionTestObj2.SendChatSignal22(0, chatMsg.c_str(), flags);
            ...
            sessionTestObj1.CancelSessionless(serial);
        } else if (cmd == "am") {
            String rule = Trim(line);
            if (rule.empty()) {
                printf("Usage: am <rule>\n");
                continue;
            }
            DoAddMatch(rule);
        } else if (cmd == "rm") {
            ...
            printf("schat1.2.2 <msg>                                              - Send a SLS using BusObject 1, Interface 2, Member 2\n");
            printf("schat2.1.1 <msg>                                              - Send a SLS using BusObject 2, Interface 1, Member 1\n");
            printf("schat2.1.2 <msg>                                              - Send a SLS using BusObject 2, Interface 1, Member 2\n");
            printf("schat2.2.1 <msg>                                              - Send a SLS using BusObject 2, Interface 2, Member 1\n");
            printf("schat2.2.2 <msg>                                              - Send a SLS using BusObject 2, Interface 2, Member 2\n");
            ...
            printf("am <rule>                                               - Add a DBUS rule\n");
            printf("rm <rule>                                            - Remove a DBUS rule\n");

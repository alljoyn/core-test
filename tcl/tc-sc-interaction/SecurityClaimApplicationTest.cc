...
class TCThread : public Thread {
  ...
  public:
    ...
    void SetUp(const char* router) {
        ...
        ASSERT_EQ(AJ_OK, AJ_FindBusAndConnect(&bus, router, TC_LEAFNODE_CONNECT_TIMEOUT));
    ...
    QStatus EnablePeerSecurity(const char* mechanisms, AuthListener* listener = NULL, const char* keystore = NULL, bool shared = FALSE) {
        QCC_UNUSED(listener);
        QCC_UNUSED(keystore);
        QCC_UNUSED(shared);
    ...
    void SetPermissionManifest(AJ_Manifest* manifest) {
        ...
        SetApplicationState(APP_STATE_NEED_UPDATE);
    }
...
TEST_F(SecurityClaimApplicationTest, fail_if_incorrect_publickey_used_in_identity_cert)
{
    ...
    /* FAIL: SC error not correct */
    EXPECT_EQ(ER_UNKNOWN_CERTIFICATE, sapWithTC.Claim(securityManagerKey,
                                                         securityManagerGuid,
                                                         securityManagerKey,
                                                         identityCertChain, 1,
                                                         manifest, manifestSize));

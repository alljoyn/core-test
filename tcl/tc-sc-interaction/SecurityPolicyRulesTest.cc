...
class TCThread : public Thread {
  ...
  public:
            ...
            if (AJ_ERR_READ == TCStatus) {
                ...
                case AJ_REPLY_ID(PRX_GET_PROP):
                    if (AJ_MSG_ERROR == msg.hdr->msgType) {
                        ...
                        switch (propid) {
                        case PRX_PROP1:
                            AJ_UnmarshalArgs(&msg, sig, &prop1);
                            printf("Get Prop1 Reply: %d\n", prop1);
                            SCStatus = ER_OK;
                            break;

                        case PRX_PROP2:
                            AJ_UnmarshalArgs(&msg, sig, &prop2);
                            printf("Get Prop2 Reply: %d\n", prop1);
                            SCStatus = ER_OK;
                            break;
                        }
                    }
    ...
    QStatus GetProperty(const char* peer, const char* mbr, int32_t& i) {
        AJ_Status status;
        AJ_Message msg;
        AJ_MarshalMethodCall(&bus, &msg, PRX_GET_PROP, peer, session, 0, 25000);
        SCStatus = ER_FAIL;
        response[0] = '\0';
        if (0 == strncmp(mbr, "Prop1", 5)) {
            propid = PRX_PROP1;
    ...
    QStatus SetProperty(const char* peer, const char* mbr, int32_t i) {
        AJ_Status status;
        AJ_Message msg;
    ...
    QStatus GetAllProperties(const char* peer) {
        AJ_Status status;
        AJ_Message msg;
        allprops.Clear();
        status = AJ_MarshalMethodCall(&bus, &msg, PRX_ALL_PROP, peer, session, 0, 25000);
        SCStatus = ER_FAIL;
        response[0] = '\0';
        if (AJ_OK != status) {
            if (AJ_ERR_ACCESS == status) {
    ...
    QStatus GetAllPropertiesReply(TCProps** props) {
        for (int msec = 0; msec < WAIT_SIGNAL; msec += WAIT_MSECS) {
            if (ER_FAIL != SCStatus) {
                break;
            }
    ...
    QStatus MethodCall(const char* peer, const char* mbr, const char* str) {
        AJ_Status status;
        AJ_Message msg;
        SCStatus = ER_FAIL;
        response[0] = '\0';
        if (0 == strncmp(mbr, "Echo", 4)) {
            status = AJ_MarshalMethodCall(&bus, &msg, PRX_ECHO, peer, session, 0, 25000);
            if (AJ_OK != status) {
                if (AJ_ERR_ACCESS == status) {
                    return ER_PERMISSION_DENIED;
                }
...
TEST_P(SecurityPolicyRulesMethodCalls, PolicyRulesTC)
{
    ...
    QStatus status = TCBus.MethodCall(SCBus.GetUniqueName().c_str(), "Echo", s);
...
TEST_P(SecurityPolicyRulesGetProperty, PolicyRulesTC)
{
    ...
    /* Create the ProxyBusObject and call the Echo method on the interface */
    int32_t prop = 513;
    QStatus status = TCBus.GetProperty(SCBus.GetUniqueName().c_str(), "Prop1", prop);
    if (GetParam().proxyObjAllowedToCallGetProperty && GetParam().busObjAllowedToRespondToGetPropertyCall) {
        EXPECT_EQ(ER_OK, status);
        //Verify we got Prop1 prop1Arg should be changed from 513 to 42 (note prop1 defaults to 42 by the constructor)
        EXPECT_EQ(42, prop);
...
TEST_P(SecurityPolicyRulesSetProperty, PolicyRulesTC)
{
    ...
    int32_t prop = 513;
    QStatus status = TCBus.SetProperty(SCBus.GetUniqueName().c_str(), "Prop1", prop);
    if (GetParam().proxyObjAllowedToCallSetProperty && GetParam().busObjAllowedToRespondToSetPropertyCall) {
        EXPECT_EQ(ER_OK, status);
        //Verify Prop1 is changed.
...
TEST_P(SecurityPolicyRulesSignal, PolicyRulesTC)
{
    ...
    QStatus status = TCBus.Signal(SCBus.GetUniqueName().c_str(), "Chirp", s);
    if (GetParam().busObjAllowedToSendSignal) {
        EXPECT_EQ(ER_OK, status);
        for (int msec = 0; msec < 2000; msec += WAIT_MSECS) {
...
TEST_F(SecurityPolicyRulesTest, GetAllProperties_test1_properties_succesfully_sent)
{
    ...
    QStatus status = TCBus.GetAllProperties(SCBus.GetUniqueName().c_str());
    EXPECT_EQ(ER_OK, status);
    TCProps* props;
    TCBus.GetAllPropertiesReply(&props);
...
TEST_F(SecurityPolicyRulesTest, PolicyRules_DENY_1_TC)
{
    ...
    // Verify Method call
    const char* s = "String that should be Echoed back.";
    EXPECT_EQ(ER_OK, TCBus.MethodCall(SCBus.GetUniqueName().c_str(), "Echo", s));
    EXPECT_STREQ(s, TCBus.GetResponse());

    // Verify Set/Get Property
    EXPECT_EQ(ER_OK, TCBus.SetProperty(SCBus.GetUniqueName().c_str(), "Prop1", 513));
    EXPECT_EQ(513, SCBusObject.ReadProp1());
    int32_t prop; 
    EXPECT_EQ(ER_OK, TCBus.GetProperty(SCBus.GetUniqueName().c_str(), "Prop1", prop));
    EXPECT_EQ(513, prop);
...
TEST_F(SecurityPolicyRulesTest, PolicyRules_DENY_9_TC)
{
    ...
    // Verify Method call
    const char* s = "String that should be Echoed back.";
    EXPECT_EQ(ER_PERMISSION_DENIED, TCBus.MethodCall(SCBus.GetUniqueName().c_str(), "Echo", s));

    // Verify Set/Get Property
    EXPECT_EQ(ER_PERMISSION_DENIED, TCBus.SetProperty(SCBus.GetUniqueName().c_str(), "Prop1", 513));
    int32_t prop; 
    EXPECT_EQ(ER_PERMISSION_DENIED, TCBus.GetProperty(SCBus.GetUniqueName().c_str(), "Prop1", prop));
